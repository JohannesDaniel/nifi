    ...
    private Object parseUnknownField(StartElement startElement, boolean dropUnknown, RecordSchema schema) throws XMLStreamException {
                ...
                if (dropUnknown) {
                    if (schema != null) {
                        ...
                        if (field.isPresent()){
                            ...
                            // subFields of subStartElement can only be known if a field is defined as record
                            // type MAP embedding fields of type RECORD is not supported for coerce == false && dropUnknown == true
        ...
        if (hasContent) {
            if (!hasFields) {
                ...
                // create record with schema if provided
            ...
            if (xmlEvent.isStartElement()) {
                ...
                if (dropUnknown) {
                    if (field.isPresent()) {
                        ...
                        // dropUnknown == true && coerceTypes == false
                        // subFields of subStartElement can only be known if a field is defined as record
                        // type MAP embedding fields of type RECORD is not supported for this case
                        } else {
                            final DataType dataType = field.get().getDataType();
                            RecordSchema childSchema = null;
